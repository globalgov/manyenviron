...
test_that("Column `Signature` has standardised dates", {
  ...
  expect_false(any(grepl("^[:digit:]{2}-[:digit:]{2}-[:digit:]{4}$", agreements[["IEADB"]]$Signature)))
  expect_false(any(grepl("^[:digit:]{1}-[:digit:]{1}-[:digit:]{4}$", agreements[["IEADB"]]$Signature)))
  expect_false(any(grepl("^[:digit:]{2}-[:digit:]{1}-[:digit:]{4}$", agreements[["IEADB"]]$Signature)))
  expect_false(any(grepl("^[:digit:]{1}-[:digit:]{2}-[:digit:]{4}$", agreements[["IEADB"]]$Signature)))
  expect_false(any(grepl("^[:digit:]{2}-[:digit:]{4}$", agreements[["IEADB"]]$Signature)))
  expect_false(any(grepl("^[:alpha:]$", agreements[["IEADB"]]$Signature)))
})
...
test_that("Columns with dates are standardized", {
  ...
  if (!is.null(agreements[["IEADB"]]$Term)) {
    ...
    expect_false(any(grepl("^[:digit:]{2}-[:digit:]{2}-[:digit:]{4}$", agreements[["IEADB"]]$Term)))
    expect_false(any(grepl("^[:digit:]{1}-[:digit:]{1}-[:digit:]{4}$", agreements[["IEADB"]]$Term)))
    expect_false(any(grepl("^[:digit:]{2}-[:digit:]{1}-[:digit:]{4}$", agreements[["IEADB"]]$Term)))
    expect_false(any(grepl("^[:digit:]{1}-[:digit:]{2}-[:digit:]{4}$", agreements[["IEADB"]]$Term)))
    expect_false(any(grepl("^[:digit:]{2}-[:digit:]{4}$", agreements[["IEADB"]]$Term)))
    expect_false(any(grepl("^[:alpha:]$", agreements[["IEADB"]]$Term)))
  }
})
