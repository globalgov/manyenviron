...
test_that("Columns are not in date, POSIXct or POSIXlt class", {
  expect_false(lubridate::is.Date(memberships[["GNEVAR_MEM"]]))
  expect_false(lubridate::is.POSIXct(memberships[["GNEVAR_MEM"]]))
  expect_false(lubridate::is.POSIXlt(memberships[["GNEVAR_MEM"]]))
})
...
test_that("dataset is arranged by the `Beg` variable", {
  expect_true(memberships[["GNEVAR_MEM"]]$Beg[1] <
                memberships[["GNEVAR_MEM"]]$Beg[10])
  expect_true(memberships[["GNEVAR_MEM"]]$Beg[50] <
                memberships[["GNEVAR_MEM"]]$Beg[75])
  expect_true(memberships[["GNEVAR_MEM"]]$Beg[100] <
                memberships[["GNEVAR_MEM"]]$Beg[120])
})
