...
references <- dplyr::left_join(references, agr_action,
  ...
  dplyr::select(treatyID1, treatyID2, RefType, known_agr, action, year,
                year_range) |>
  dplyr::distinct()
# get titles for hovering
titles <- manyenviron::agreements$ECOLEX |> 
  dplyr::select(manyID, Title) |>
  dplyr::bind_rows(dplyr::select(manyenviron::agreements$HUGGO, manyID, Title)) |>
  ...
  filteredData <- shiny::reactive({
    ...
    output$hover_info <- renderUI({
      ...
      wellPanel(style = "position:relative; background: #F0F8FF; border-color: #FFFFFF; ",
                if (titlet == "character(0)") "Hover over nodes to see the treaty title" else titlet,
