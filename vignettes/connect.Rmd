...
set a node attribute `lvl` identifying the set a node belongs to. We then move on to
subsetting the graph by only retaining the biggest component and applying the
`layout_as_multilevel()` function. We finally make some cosmetic
...
graphjs(membership_1980,
        main = "Membership in Environmental Treaties Signed in 1980",
        bg = "white",
...
As a little bonus example, let's look at what the "country projection" of
the bimodal network above looks like on a globe. Before, we start, note that
projections such as the ones we're about to perform are obscuring information
about the underlying bimodal network. Now that we mentioned this caveat, let's
look at how we can create this visualisation.

There are essentially two elements we need for this visualization. First,
the country projection of reaties signed in 1980 which we computed before.
Second, a dataframe of the centroids of every country in the world which we get
from the `{rgeos}` and the `{rworldmap}` packages. Once we have these elements,
we merge them and create a specific dataframe for the arcs to be displayed in
the `globejs()` visualization. Finally, we plot it and adjust colors and
inital longitude and latitude of the plotted globe.
...
membership_1980 <- project_cols(membership_1980)
membership_1980 <- igraph::get.data.frame(membership_1980,
                                                "edges")
# Step 2: Get the coordinates of the centroids of each territory
...
# Step 3: Merge centroids and links dataframes
