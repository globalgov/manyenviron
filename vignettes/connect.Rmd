...
### Explore the data composition and sources with `{qData}` functions

To have a quick overview of each of the database and dataset within the `{manyenviron}`, the function `data_contrast()` from the `{qData}` package return a table with useful information such as number of unique ID, the percentage of missing data, the number of rows and columns, the earliest Beg date, the oldest End date, as well as the URL link to the data source website.
The function allows the user to choose between displaying these information for only one dataset, or for the 
whole database, as showed in the example below. 
```{r, eval=FALSE}
qData::data_contrast("manyenviron", "agreements", "IEADB")
qData::data_contrast("manyenviron", "memberships")
```

Another useful function from `{qData}` package to contrast information from `{manyenviron}` databases is `code_source()`, which displays bibliographic references for a dataset, a database or the whole package.
```{r, eval=FALSE}
...
To facilitate the identification and connection between the databases in the package, the `manypkgs::code_agreements()` function has been developed to generate a qID for treaties in a database.
Each of the `{manyenviron}` dataset contains a qID column.
