...
IEA <- readr::read_csv("data-raw/members/IEA/iea_membs.csv")

# Stage two: Correcting data
# In this stage you will want to correct the variable names and
# formats of the 'IEA' object until the object created
# below (in stage three) passes all the tests. 
IEA <- as_tibble(IEA) %>%
  filter(!str_detect(country, pattern = "IO")) %>% 
  transmutate(ID = {country},
              Title = standardise_titles(`Agreement Name`),
              Signature = `Agreement Signature`,
              Force = `Entry into force`) %>%
...
# Next run the following line to make IEA available within the qPackage.
export_data(IEA, database = "members")
