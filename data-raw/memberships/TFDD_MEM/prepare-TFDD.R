...
TFDD_MEM <- as_tibble(TFDD_MEM) %>%
  dplyr::mutate(Signature = manypkgs::messydates::make_messydates(openxlsx::convertToDate(DateSigned))) %>%
  dplyr::mutate(Beg = manypkgs::messydates::make_messydates(as.character(Signature))) %>%
